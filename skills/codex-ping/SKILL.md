...
1. If the recipient is unclear, run `./hw 在线 --json` and use the returned
   recently active identities. Ask the user when more than one plausible target
   remains.
2. Send a message with `./hw <recipient><message>`, for example
   `./hw 小明测试完成了吗？`.
3. Read pending messages with `./hw 收 --json`. Reading burns the messages.
